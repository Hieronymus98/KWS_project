...
def build_model(input_shape, learning_rate, loss = "sparse_categorical_crossentropy"):
    ...
    model.add(tf.keras.layers.GRU(units=154, input_shape=input_shape, return_sequences=False))
    # if RNN layer is not one layer, 
    # return_seqeunce(Whether to return the last state in addition to the output. )
    # is True, and default is False
    
    model.add(tf.keras.layers.Dense(118, activation='relu'))
